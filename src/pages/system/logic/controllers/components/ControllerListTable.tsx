...
import {Button, Col, Descriptions, Popconfirm, Popover, Row, Space, Table, Tag, Tooltip, Typography} from "antd";
import {EmptyText, ModulePopover, PageAlert, SmartTime} from "@/huikedev";
import {AppstoreAddOutlined, DeleteOutlined, EditOutlined, ReloadOutlined} from "@ant-design/icons";
...
const ControllerListTable: React.FC = () => {
  ...
  const ControllerColumns: ColumnsType<API.ControllerTableItem> = [
    ...
    },
    {
      ...
      key: 'controller_name',
      render: (res, record) => {
        return <Tooltip title={`app\\controller\\${record.module.module_name}\\${record.path.controller_name}\\${record.controller_name}`}>
          <Space>
    ...
    },
    {
      ...
      align:'center',
      render: (res, record) => {
        return <Tooltip title={record.service_class}><span className={publicStyle.textLink}>{getClassName(record.service_class)}</span></Tooltip>
      }
    },{
      title: '代理模式',
          ...
          {record.actions.length > 0 ?
            <Popover
              ...
              content={<Space direction="vertical" style={{width:'100%'}}>
                {record.actions.length > 0 && (record.actions as Array<API.ActionItemWithAppends>).map((action) => {
                  return <Row key={action.action_name}>
      ...
      align:'center',
      render:(res,record)=>{
        return record.created_by_huike ? <span className={publicStyle.textOrange}>系统</span> : <span>手动</span>;
      }
    },
    {
      title: '创建人',
      ...
      render:(res,record)=>{
        return (<Space>
          <Button
            ...
            icon={<EditOutlined />}
          />
          {record.is_static_service
            ?
            null
            :
            <Popconfirm title="是否确认更新服务门面" onConfirm={()=>updateFacadeSubmit({controller_id:record.id})}>
              <Tooltip title="更新服务门面">
                <Button loading={updateFacadeSubmitting} className={publicStyle.btnGenerateOutline} icon={<ReloadOutlined />} />
              </Tooltip>
            </Popconfirm>
          }
  ...
  return (
    <Space direction="vertical" size={15} style={{width: '100%'}}>
      ...
      />
      <Space size={10}>
        <Button type="primary" onClick={() => history.push(PathName.controllers.create)}>添加控制器</Button>
        <Button className={publicStyle.btnWarning} type="primary"
                onClick={() => history.push(PathName.controllers.sync)}>同步控制器</Button>
      </Space>
