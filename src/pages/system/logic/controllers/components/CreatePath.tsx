...
import { setControllerPathCreate } from '@/services/logic';
...
const CreatePath: React.FC<CreatePathProps> = (props) => {
  ...
  const { run: submit, loading: submitting } = useRequest(
    (params: object) => setControllerPathCreate(params),
    {
      manual: true,
    },
  );
  ...
  return (
    <Space key="controller_path" direction="vertical" size={20} style={{ width: '100%' }}>
      <PageAlert
        closable
        message={
          <div>
            <p>
              目录名称不支持多级自动关联，如您创建<Text code>path/sub_path</Text>
              ，系统会认为这是一个目录，您无法将控制器生成在<Text code>path</Text>
              目录下，如需生成则需要添加<Text code>path</Text>目录
            </p>
            <p>
              <Text type="danger">目录名称会自动转换为Snake命名风格</Text>
              ，目录名称也将作为Service的路径，所以目录的意义是对业务逻辑进行分类管理，方便后续的开发及维护
            </p>
          </div>
        }
      />
      ...
      >
        ...
        >
          <Select placeholder="选择模块">
            {props.moduleList.map((module) => {
              return (
                <Select.Option value={module.id} key={module.name}>
                  {module.title}
                </Select.Option>
              );
            })}
        ...
        >
          <Input
            placeholder="请输入目录名称（英文）"
            onChange={(e) => {
              onControllerNameChange(e.target.value as string);
            }}
          />
        </Form.Item>
        <Form.Item
