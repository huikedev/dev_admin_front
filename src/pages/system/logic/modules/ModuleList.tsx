...
const ModuleList: React.FC = () => {
  const { btnLock, result, fetchModuleList, refreshRoutes, refreshException } = useModel(
    'useModuleModel',
    (model) => ({
      btnLock: model.btnLock,
      result: model.result,
      fetchModuleList: model.fetchModuleList,
      refreshRoutes: model.refreshRoutes,
      refreshException: model.refreshException,
    }),
  );
        ...
        if (record.extend_module_id > 0) {
          return (
            <Space>
              ...
              <Tooltip title="第三方模块">
                <AppstoreAddOutlined className={publicStyle.textBlue} size={18} />
              </Tooltip>
            </Space>
          );
        }
              ...
              content={
                <div style={{ width: '400px' }}>
                  <Descriptions column={1}>
                    <Descriptions.Item label="根命名空间">
                      <Text code>{record.extend_module?.root_namespace}</Text>
                    </Descriptions.Item>
                    <Descriptions.Item label="模块根路径">
                      <Text code>{record.extend_module?.root_path}</Text>
                    </Descriptions.Item>
                  </Descriptions>
                </div>
              }
      ...
      render: (res: any, record) => {
        return (
          <Space>
            ...
            <Popconfirm
              key="migrateRun"
              title={`确认更新【${record.module_title}】的路由规则?`}
              onConfirm={async () => {
                await refreshRoutes(record.id);
              }}
              okText="确认"
              cancelText="取消"
            >
              <Tooltip title="更新路由规则">
                <Button
            ...
            <Popconfirm
              ...
              title="确认更新异常配置文件?"
              onConfirm={async () => {
                await refreshException(record.id);
              }}
              okText="确认"
              cancelText="取消"
            >
              <Tooltip title="更新异常配置文件">
                <Button
    ...
    <PageContainer>
      <Card>
        <Space direction="vertical" size={20} style={{ width: '100%' }}>
          ...
          <Table key="moduleList" columns={ModuleColumns} rowKey="id" {...tableProps} />
          {showModuleEditModal()}
        </Space>
      </Card>
    </PageContainer>
