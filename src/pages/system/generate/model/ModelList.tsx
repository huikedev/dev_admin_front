...
import {FileSearchOutlined, SwapOutlined, NodeExpandOutlined, DeleteOutlined,DownOutlined} from '@ant-design/icons'
  ...
  const handleMigrationDropdown = async (key:string | number,modelId:number) => {
    switch (key){
      case 'runMigration':
        Modal.confirm({
          content:'是否确认执行数据库迁移？',
          onOk:async ()=>{await migrateRun({id:modelId})}
        })
        break;
      case 'tableToMigration':
        Modal.confirm({
          content:'是否确认根据表字段生成数据迁移文件（若存在会覆盖）？',
          onOk:async ()=>{await tableToMigration({id:modelId})}
        })
        break;
      case 'tableToSeeds':
        Modal.confirm({
          content:'是否确认根据表数据生成数据种子文件（若有旧文件请手动删除）？',
          onOk:async ()=>{await tableToSeeds({id:modelId})}
        })
        break;
      default:
        Modal.error({
    ...
    },
    {
      ...
      render:(res:any,record: ModelListDataType)=>{
        return <Space>
          ...
          <Tooltip title="查看属性">
            <FileSearchOutlined className={publicStyle.textBlue} onClick={()=>{
              setModelData(record)
      ...
      render:(res:any,record: ModelListDataType)=>{
        ...
        if(record.model_fields_count === 0) {
          if (record.migrate_file.length===0) {
            return <Link to={{pathname: `${PathName.generate.modelFieldsAdd}/${record.id}`}}>添加</Link>
          }
        ...
        return <Space>
          ...
          <Dropdown trigger={['click']} placement="bottomRight" overlay={
            <Menu onClick={({ key})=>{
              ...
              <Menu.Item key="runMigration" disabled={record.migrate_file.length === 0 || record.migrate_version.length > 0}>
                执行数据库迁移
              </Menu.Item>
              <Menu.Item key="tableToMigration">
                表字段生成迁移文件
              </Menu.Item>
              <Menu.Item key="tableToSeeds">
                表数据生成种子文件
              </Menu.Item>
            </Menu>
          }>
        ...
        return <Space>
          ...
          >
            <Tooltip title="更新模型注解">
              <Button
                className={publicStyle.btnGreenOutline}
    ...
    return (
      ...
      <Space direction="vertical" size={20} style={{width:'100%'}}>
        <PageAlert closable message={<div>
          <p>若需要重新执行数据库迁移，请手动删除数据表migrations中对应的记录，并将huike_model_list中模型对应的migrate_version修改为null</p>
          <p>模型生成只支持新建模型，不支持模型属性修改或更新，后续可手动修改</p>
        </div>}
        />
