  ...
  const buildFieldRow = (
    ...
    return (
      <Row key={field.key}>
        ...
        <Col span={3}>
          <Form.Item {...field} name={[field.name, 'type']} fieldKey={[field.fieldKey, 'type']}>
            <Select
              showSearch
              placeholder="字段类型"
              style={{ width: '100%' }}
            ...
            >
              {modelFieldTypes.map((group) => {
                return (
                  <Select.OptGroup key={group.name} label={group.title}>
                    {group.children.map((fieldType) => {
                      return (
                        <Select.Option key={fieldType.name} value={fieldType.name}>
                          {fieldType.title}
                        </Select.Option>
                      );
                    })}
        ...
        <Col span={2}>
          <Form.Item
            ...
            shouldUpdate={(prevValues, currentValues) => {
              ...
              return (
                prevValues.model_fields[field.fieldKey].type !==
                currentValues.model_fields[field.fieldKey].type
              );
            }}
          >
            {() => {
              ...
              let fieldType = 'none';
              if (
                typeof fieldsValue.model_fields !== 'undefined' &&
                typeof fieldsValue.model_fields[field.fieldKey] !== 'undefined'
              ) {
          ...
          >
            <Select placeholder="索引类型" style={{ width: '100%' }} disabled={disable}>
              {modelFiledIndexType.map((indexType) => {
                return (
                  <Select.Option key={indexType.name} value={indexType.name}>
                    {indexType.title}
                  </Select.Option>
                );
              })}
        ...
        <Col span={5}>
          ...
          >
            <Checkbox.Group>
              {isPk ? (
                <Checkbox value="primary" checked disabled>
                  PRIMARY
                </Checkbox>
              ) : null}
              <Checkbox value="nullable" disabled={disable}>
                NULL
              </Checkbox>
              <Checkbox value="unsigned" disabled={disable}>
                非负
              </Checkbox>
            </Checkbox.Group>
          </Form.Item>
        ...
        <Space direction="vertical" style={{ width: '100%' }}>
          <PageAlert
            message={
              <div>
                <p>
                  数据表生成通过think-migration完成，仅提供常用字段类型。字段长度以think-migration生成的字段长度为准，除特殊字段外无法指定
                </p>
                <p>
                  若<Text code>char</Text>、<Text code>varchar</Text>
                  类型需要设置空字符串的默认值，请在默认值填写<Text code>EMPTY STRING</Text>
                </p>
              </div>
            }
          />
          ...
          >
            ...
            <Form.List name="primary">
              {(fields) => <>{fields.map((field) => buildFieldRow(field, true))}</>}
            </Form.List>
            {/* 自定义字段 */}
            ...
            {formInitData.timestamp_fields.length > 0 ? (
              <Form.List name="timestamp_fields">
                {(fields) => <>{fields.map((field) => buildFieldRow(field, false))}</>}
              </Form.List>
            ) : null}
